...
defaultValue:"");
  static Future<String> scanAndRecognizeText() async {
    ...
    if(isMobile){
      ...
      final cams=await availableCameras(); final cam=cams.first;
      final c=CameraController(cam, ResolutionPreset.high, 
enableAudio:false); await c.initialize();
      final pic=await c.takePicture(); await c.dispose();
      final input=InputImage.fromFilePath(pic.path); final 
rec=TextRecognizer(); final res=await rec.processImage(input); await 
rec.close();
