...
resource "google_project_iam_member" "run_admin_prod" { count=var.assign_deploy_roles?1:0 project=var.project_id role="roles/run.admin" member="serviceAccount:${google_service_account.deploy_prod.email}" }
resource "google_artifact_registry_repository" "repo" { location=var.region repository_id=var.ar_repository_id format="DOCKER" description="FoodLabel AI docker repo" }
